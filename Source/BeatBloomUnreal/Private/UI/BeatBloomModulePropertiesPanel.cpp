...
void SBeatBloomModulePropertiesPanel::CreatePropertyWidgets() {
    ...
    Container->AddSlot().AutoHeight().Padding(5.0f)
        [SNew(SHorizontalBox) +
         SHorizontalBox::Slot().FillWidth(1.0f).Padding(
             0.0f, 0.0f, 5.0f,
             0.0f)[FCommonPanelUtility::CreateFilePathPropertyRowWithCallback(
             TEXT("DrumKit Config Path"), BeatBloom->DrumKitConfigPath,
             TEXT("DrumKitConfigPath"), TEXT(".drumkit"),
         ...
         SHorizontalBox::Slot().AutoWidth()
             [SNew(SButton)
                  ...
                  .OnClicked(
                      this,
                      &SBeatBloomModulePropertiesPanel::OnLoadDrumKitConfig)
    ...
    if (BeatBloom->DrumKitConfig.Components.Num() > 0) {
        ...
        Container->AddSlot().AutoHeight().Padding(
            ...
            TEXT("Configuration Name"), BeatBloom->DrumKitConfig.Name,
            TEXT("DrumKitConfigName"), nullptr)];

        // 肢体数量
        Container->AddSlot().AutoHeight().Padding(
            5.0f)[FCommonPanelUtility::CreateNumericPropertyRow(
            TEXT("Limb Count"), BeatBloom->DrumKitConfig.Limbs.Num(),
            TEXT("LimbCount"),
        ...
        Container->AddSlot().AutoHeight().Padding(
            5.0f)[FCommonPanelUtility::CreateNumericPropertyRow(
            TEXT("Component Count"), BeatBloom->DrumKitConfig.Components.Num(),
            TEXT("ComponentCount"),
            [this](const FString& PropertyPath, int32 NewValue) {
                OnNumericPropertyChanged(PropertyPath, NewValue);
            })];
    ...
    Container->AddSlot().AutoHeight().Padding(
        5.0f)[SNew(SButton)
                  .Text(LOCTEXT("CheckObjectsStatusButton",
                                "Check Drummer Control Rig Status"))
                  .OnClicked(
                      this,
    ...
    Container->AddSlot().AutoHeight().Padding(
        5.0f)[SNew(SButton)
                  .Text(LOCTEXT("SetupAllObjectsButton",
                                "Setup Drummer Control Rig"))
                  .OnClicked(
                      this, &SBeatBloomModulePropertiesPanel::OnSetupAllObjects)
                  .HAlign(HAlign_Center)
                  .ButtonStyle(FAppStyle::Get(), "FlatButton.Default")];
    ...
    Container->AddSlot().AutoHeight().Padding(
        ...
        TEXT("IO File Path"), BeatBloom->IOFilePath, TEXT("IOFilePath"),
        TEXT(".drummer"),
        [this](const FString& NewPath) {
            if (BeatBloomActor.IsValid()) {
                BeatBloomActor.Get()->Modify();
                ...
                UE_LOG(LogTemp, Warning,
                       TEXT("BeatBloom: IO File Path updated to: %s"),
                       *NewPath);
            }
        },
        true)];
    ...
    Container->AddSlot().AutoHeight().Padding(5.0f)
        [SNew(SButton)
             ...
             .OnClicked(this,
                        &SBeatBloomModulePropertiesPanel::OnExportRecorderInfo)
             .HAlign(HAlign_Center)
             .ButtonStyle(FAppStyle::Get(), "FlatButton.Default")];
...
void SBeatBloomModulePropertiesPanel::OnNumericPropertyChanged(
    ...
    UE_LOG(LogTemp, Warning,
           TEXT("BeatBloom: Numeric property change attempted for %s = %d "
                "(read-only)"),
...
void SBeatBloomModulePropertiesPanel::OnFilePathChanged(
    ...
    if (PropertyPath == TEXT("DrumKitConfigPath")) {
        ...
        BeatBloom->Modify();
        UE_LOG(LogTemp, Warning,
               TEXT("BeatBloom: DrumKit Config Path updated to: %s"),
               *NewFilePath);
    }
}
...
FReply SBeatBloomModulePropertiesPanel::OnLoadDrumKitConfig() {
    ...
    // 如果路径为空，弹出文件选择对话框
    if (FilePath.IsEmpty()) {
        if (FCommonPanelUtility::BrowseForFile(TEXT(".drumkit"), FilePath,
                                               false)) {
    ...
    if (!FPaths::FileExists(FilePath)) {
        UE_LOG(LogTemp, Error,
               TEXT("BeatBloom: DrumKit config file does not exist: %s"),
               *FilePath);
        return FReply::Handled();
    }
    ...
    if (BeatBloom->LoadDrumKitConfig(FilePath)) {
        UE_LOG(LogTemp, Warning,
               TEXT("BeatBloom: Successfully loaded drumkit config from %s"),
               *FilePath);
        ...
        UE_LOG(LogTemp, Error,
               TEXT("BeatBloom: Failed to load drumkit config from %s"),
               *FilePath);
    }
...
FReply SBeatBloomModulePropertiesPanel::OnCheckObjectsStatus() {
    if (!BeatBloomActor.IsValid()) {
        UE_LOG(LogTemp, Error,
               TEXT("BeatBloom: No actor selected for check drummer control "
                    "rig status"));
        return FReply::Handled();
    }
    ...
    UE_LOG(
        LogTemp, Warning,
        TEXT(
            "BeatBloom: Check Drummer Control Rig Status operation triggered"));
    return FReply::Handled();
}
...
FReply SBeatBloomModulePropertiesPanel::OnExportRecorderInfo() {
    if (!BeatBloomActor.IsValid()) {
        UE_LOG(LogTemp, Error,
               TEXT("BeatBloom: No actor selected for export drummer info"));
        return FReply::Handled();
    }
    ...
    if (BeatBloom->IOFilePath.IsEmpty()) {
        UE_LOG(LogTemp, Error,
               TEXT("BeatBloom: IO File Path is not set. Please set IO File "
                    "Path first."));
        return FReply::Handled();
    }
    ...
    UE_LOG(LogTemp, Warning, TEXT("BeatBloom: Export Drummer Info to %s"),
...
FReply SBeatBloomModulePropertiesPanel::OnImportRecorderInfo() {
    if (!BeatBloomActor.IsValid()) {
        UE_LOG(LogTemp, Error,
               TEXT("BeatBloom: No actor selected for import drummer info"));
        return FReply::Handled();
    }
    ...
    if (!FPaths::FileExists(BeatBloom->IOFilePath)) {
        UE_LOG(LogTemp, Error, TEXT("BeatBloom: IO file does not exist: %s"),
               *BeatBloom->IOFilePath);
        return FReply::Handled();
    }
    ...
    if (BeatBloom->ImportRecorderInfo(BeatBloom->IOFilePath)) {
        UE_LOG(LogTemp, Warning, TEXT("BeatBloom: Import Drummer Info from %s"),
               *BeatBloom->IOFilePath);
        RefreshProperties();
    } else {
        UE_LOG(LogTemp, Error,
               TEXT("BeatBloom: Failed to import drummer info from %s"),
               *BeatBloom->IOFilePath);
    }
