...
void UBeatBloomDrumKitProcessor::InitializeDrumKit(
    ...
    if (!BeatBloomActor) {
        UE_LOG(LogTemp, Error,
               TEXT("BeatBloomActor is null in InitializeDrumKit"));
        return;
    }

    if (!BeatBloomActor->DrumKit) {
        UE_LOG(LogTemp, Error,
               TEXT("DrumKit is not assigned in BeatBloomActor"));
        return;
    }
    ...
    if (!ControlRigInstance || !ControlRigBlueprint) {
        UE_LOG(LogTemp, Error, TEXT("Failed to get ControlRig for DrumKit"));
        return;
    }
...
void UBeatBloomDrumKitProcessor::GenerateDrumKitAnimationFromPath(
    ABeatBloomUnreal* BeatBloomActor, const FString& ShapeKeyAnimationPath) {
    if (!BeatBloomActor) {
    ...
    for (const auto& DrumKitValue : DrumKitDataArray) {
        ...
        FString MorphTargetName =
            FString::Printf(TEXT("%s_beat"), *DrumKitName);

        // 获取 animation_data
        if (!DrumKitObject->HasField(TEXT("animation_data"))) {
            UE_LOG(LogTemp, Warning,
                   TEXT("Drum kit %s has no animation_data, skipping"),
                   *DrumKitName);
            continue;
        }

        auto AnimationDataArray =
            DrumKitObject->GetArrayField(TEXT("animation_data"));

        if (AnimationDataArray.Num() == 0) {
            UE_LOG(LogTemp, Warning,
                   TEXT("Drum kit %s has empty animation_data, skipping"),
                   *DrumKitName);
            continue;
        }
        ...
        for (const auto& FrameValue : AnimationDataArray) {
            ...
            double FrameNumberDouble =
                FrameObject->GetNumberField(TEXT("frame"));
            float Value =
                static_cast<float>(FrameObject->GetNumberField(TEXT("value")));

            // 转换帧编号
            float ScaledFrameNumberFloat = FrameNumberDouble *
...
USkeletalMeshComponent* UBeatBloomDrumKitProcessor::FindMorphTargetOwner(
    ABeatBloomUnreal* BeatBloomActor, const FString& MorphTargetName) {
    ...
    if (SkeletalMeshComp->GetSkeletalMeshAsset()) {
        const TArray<UMorphTarget*>& MorphTargets =
            SkeletalMeshComp->GetSkeletalMeshAsset()->GetMorphTargets();
...
bool UBeatBloomDrumKitProcessor::WriteDrumKitMorphTargetAnimation(
    ABeatBloomUnreal* BeatBloomActor, ULevelSequence* LevelSequence,
    ASkeletalMeshActor* DrumKitActor, const FString& MorphTargetName,
    ...
    if (AnimationDataArray.Num() == 0) {
        UE_LOG(LogTemp, Warning, TEXT("Empty animation data for %s"),
               *MorphTargetName);
        return false;
    }
    ...
    if (!SkeletalMeshComp) {
        UE_LOG(LogTemp, Error, TEXT("MorphTarget %s not found"),
               *MorphTargetName);
        return false;
    }
    ...
    if (KeyframeData.FrameNumbers.Num() == 0) {
        UE_LOG(LogTemp, Warning, TEXT("No valid frames for %s"),
               *MorphTargetName);
        return false;
    }
    ...
    int32 WrittenCount =
        UInstrumentMorphTargetUtility::WriteMorphTargetAnimationToControlRig(
            DrumKitActor, SingleKeyframeData, LevelSequence,
            TEXT("drumkit_control"));
