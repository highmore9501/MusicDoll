...
void UBeatBloomAnimationProcessor::GeneratePerformerAnimation(
    ...
    if (!FFileHelper::LoadFileToString(FileContent, *PerformerAnimationPath)) {
        UE_LOG(LogTemp, Error, TEXT("Failed to load animation file: %s"), *PerformerAnimationPath);
        return;
    }
    ...
    if (!FJsonSerializer::Deserialize(Reader, JsonObject)) {
        UE_LOG(LogTemp, Error, TEXT("Failed to parse animation JSON: %s"), *PerformerAnimationPath);
        return;
    }
    ...
    // 获取 ControlRig 缓存
    UControlRig* ControlRigInstance = BeatBloomActor->GetCachedControlRig(TEXT("Performer"));
    if (!ControlRigInstance) {
        UE_LOG(LogTemp, Error, TEXT("Failed to get ControlRig instance for Performer"));
        return;
    }
    
    // 清除旧关键帧
    UInstrumentAnimationUtility::CleanupInstrumentAnimationTracks(
        BeatBloomActor->SkeletalMeshActor);
    ...
    if (JsonObject->HasField(TEXT("left_hand_animation"))) {
        auto LeftHandArray = JsonObject->GetArrayField(TEXT("left_hand_animation"));
    ...
    if (JsonObject->HasField(TEXT("right_hand_animation"))) {
        auto RightHandArray = JsonObject->GetArrayField(TEXT("right_hand_animation"));
    ...
    if (JsonObject->HasField(TEXT("left_foot_animation"))) {
        auto LeftFootArray = JsonObject->GetArrayField(TEXT("left_foot_animation"));
    ...
    if (JsonObject->HasField(TEXT("right_foot_animation"))) {
        auto RightFootArray = JsonObject->GetArrayField(TEXT("right_foot_animation"));
    ...
    // 刷新显示
    BeatBloomActor->TriggerControlRigReregistration(TEXT("Generate Performer Animation"));
    
    UE_LOG(LogTemp, Warning, TEXT("========== GeneratePerformerAnimation Summary =========="));
    UE_LOG(LogTemp, Warning, TEXT("Successfully processed: %d frames"), ProcessedFrames);
    UE_LOG(LogTemp, Warning, TEXT("Total keyframes added: %d"), KeyframesAdded);
    UE_LOG(LogTemp, Warning, TEXT("========== GeneratePerformerAnimation Completed =========="));
}
    ...
    for (const auto& FrameValue : AnimationArray) {
        ...
        if (FrameObject->HasField(TEXT("position"))) {
            ...
            if (FrameObject->HasField(TEXT("rotation"))) {
                auto RotationArray = FrameObject->GetArrayField(TEXT("rotation"));
                FQuat Rotation;
            ...
            ControlKeyframeData.FindOrAdd(FString::Printf(TEXT("H_%s"), *HandSuffix)).Add(Keyframe);
            OutKeyframesAdded++;
        }
        
        // 处理 pivot_position -> HP_{Suffix}（只位置）
        if (FrameObject->HasField(TEXT("pivot_position"))) {
            auto PivotPositionArray = FrameObject->GetArrayField(TEXT("pivot_position"));
            ...
            ControlKeyframeData.FindOrAdd(FString::Printf(TEXT("HP_%s"), *HandSuffix)).Add(PivotKeyframe);
            OutKeyframesAdded++;
        }
...
void UBeatBloomAnimationProcessor::ProcessFootAnimation(
    ...
    for (const auto& FrameValue : AnimationArray) {
        ...
        if (FrameObject->HasField(TEXT("position"))) {
            ...
            ControlKeyframeData.FindOrAdd(FString::Printf(TEXT("F_%s"), *FootSuffix)).Add(Keyframe);
            OutKeyframesAdded++;
        }
