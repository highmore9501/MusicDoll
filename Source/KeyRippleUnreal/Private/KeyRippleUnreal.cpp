...
void AKeyRippleUnreal::InitializeControllersAndRecorders() {
    ...
    KeyBoardPositions.Add(TEXT("lowest_white_key_position_end"),
                          TEXT("lowest_white_key_end"));
    ...
    TargetPoints.Empty();
    TargetPoints.Add(TEXT("head_position"), TEXT("Head_Control"));
    TargetPoints.Add(TEXT("head_look_target"), TEXT("Look_At"));
    TargetPoints.Add(TEXT("middle_hand_position"), TEXT("Mid_Hand"));

    TargetPointsRecorders.Empty();

    TArray<FString> middle_hand_recorders;
    TArray<FString> head_position_recorders;
    ...
    for (const auto& pair : TargetPoints) {
        ...
        for (EKeyType key_type : {EKeyType::WHITE, EKeyType::BLACK}) {
            for (EPositionType position_type :
                ...
                if (controller_name.Contains(TEXT("middle_hand"))) {
                    middle_hand_recorders.Add(recorder_name);
                } else if (controller_name.Contains(TEXT("head"))) {
                    head_position_recorders.Add(recorder_name);
                }
            }
        }
    }

    FStringArray head_position_recorders_wrapper;
    head_position_recorders_wrapper.Strings = head_position_recorders;
    FStringArray middle_hand_recorders_wrapper;
    middle_hand_recorders_wrapper.Strings = middle_hand_recorders;

    TargetPointsRecorders.Add(TEXT("head_position_recorders"),
                              head_position_recorders_wrapper);
    TargetPointsRecorders.Add(TEXT("middle_hand_recorders"),
                              middle_hand_recorders_wrapper);
