#include "Nodes/ArcDistributedIK.h"
...
FRigUnit_ArcDistributedIK_Execute() {
    ...
    if (bUseDebug) {
        UE_LOG(LogControlRig, Warning,
               TEXT("[ArcDistributedIK] Validating chain before writing to "
                    "hierarchy"));
    }
    ...
    for (int32 i = 0; i < Chain.Num(); ++i) {
        ...
        FVector OriginalPos =
            Hierarchy->GetGlobalTransform(WorkData.CachedItems[i].GetKey())
                .GetLocation();
        FVector NewPos = Chain[i].Transform.GetLocation();
            ...
            if (bUseDebug && i < 3) {
                UE_LOG(LogControlRig, Warning,
                       TEXT("[ArcDistributedIK] Position changed for bone %d: "
                            "(%.2f, %.2f, %.2f) -> (%.2f, %.2f, %.2f)"),
                       i, OriginalPos.X, OriginalPos.Y, OriginalPos.Z, NewPos.X,
                       NewPos.Y, NewPos.Z);
            }
        }
    }
