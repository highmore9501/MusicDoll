...
bool FControlInitTransformUtility::ApplySelectedControlsTransformToOffset(
    ...
    if (!LevelSequence) {
        UE_LOG(LogTemp, Warning,
               TEXT("ApplySelectedControlsTransformToOffset: "
    ...
    for (const FControlRigSequencerBindingProxy& Proxy : RigBindings) {
        ...
        for (const FRigElementKey& Key : SelectedKeys) {
            ...
            FTransform CurrentOffset =
                RuntimeHierarchy->GetControlOffsetTransform(
            ...
            UE_LOG(LogTemp, Log,
                   TEXT("ApplySelectedControlsTransformToOffset: "
                        "Applied offset transform for Control '%s' in "
                        "Blueprint '%s'"),
...
bool FControlInitTransformUtility::ApplySelectedControlsTransformToInitial(
    ...
    for (const FControlRigSequencerBindingProxy& Proxy : RigBindings) {
        ...
        for (const FRigElementKey& Key : SelectedKeys) {
            ...
            // 5. 将实际的 Local Transform 写入 Blueprint Hierarchy 的 Initial
            // Local Transform
            constexpr bool bAffectChildren = true;
            constexpr bool bSetupUndo = true;
            constexpr bool bPrintPythonCommands = false;

            // 设置 Initial Local Transform 为实际变换（写入初始值）
            BlueprintHierarchy->SetInitialLocalTransform(
                Key, ActualLocalTransform, bAffectChildren, bSetupUndo,
                bPrintPythonCommands);

            // 将 Offset 清零
            BlueprintHierarchy->SetControlOffsetTransform(
                Key, FTransform::Identity, true, bAffectChildren, bSetupUndo,
                bPrintPythonCommands);
