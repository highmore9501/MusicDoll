...
void SModuleMainPanelBase::InitializeModulePanel(
    ...
    ChildSlot[SNew(SVerticalBox)
              ...
              + SVerticalBox::Slot().AutoHeight().Padding(5.0f)
                    [SAssignNew(TabButtonsContainer, SHorizontalBox)]
              // Content Container
...
void SModuleMainPanelBase::RegisterPanel(TSharedPtr<SWidget> InPanel, const FText& InTabLabel) {
    if (!InPanel.IsValid()) {
        return;
    }
    ...
    if (TabButtonsContainer.IsValid()) {
        TabButtonsContainer->AddSlot().FillWidth(1.0f).Padding(2.5f, 0.0f)
            [TabButton.ToSharedRef()];
    }
    
    // 不再自动显示第一个面板，避免UI未完全构建时的问题
}
...
TSharedPtr<SVerticalBox> SModuleMainPanelBase::GetPanelContainer(int32 Index) const {
...
void SModuleMainPanelBase::ShowPanel(int32 PanelIndex) {
    if (!RegisteredPanels.IsValidIndex(PanelIndex) || ActiveTab == PanelIndex) {
        return;
    }
    
    ActiveTab = PanelIndex;
    
    if (ContentContainer.IsValid() && RegisteredPanels[PanelIndex].Container.IsValid()) {
        ContentContainer->ClearChildren();
        ContentContainer->AddSlot().FillHeight(1.0f)
            [SNew(SScrollBox) +
             SScrollBox::Slot()[RegisteredPanels[PanelIndex].Container.ToSharedRef()]];
    }
}
