...
struct MUSICDOLLCOMMON_API FAnimationKeyframe {
    GENERATED_BODY()
    ...
    FAnimationKeyframe(int32 InFrameNumber, const FVector& InTranslation,
...
struct MUSICDOLLCOMMON_API FBatchInsertKeyframesSettings {
    ...
    FBatchInsertKeyframesSettings()
        : FramePadding(600), bUnwrapRotationInterpolation(true) {}
};
 ...
 */
USTRUCT(BlueprintType)
struct MUSICDOLLCOMMON_API FMaterialParameterKeyframeData {
    GENERATED_BODY()
    ...
    FMaterialParameterKeyframeData() : ParameterName(TEXT("")) {}
 ...
 * 包含旋转处理、通道管理、Component Material
 * Track管理等与乐器类型无关的通用算法
 */
UCLASS()
class MUSICDOLLCOMMON_API UInstrumentAnimationUtility : public UObject {
    GENERATED_BODY()
   ...
   public:
    ...
    /**
     * 获取当前打开的 Level Sequence
     * @return 当前LevelSequence，如果未找到则返回nullptr
     */
    static ULevelSequence* GetCurrentLevelSequence();

    /**
     * 根据SkeletalMeshComponent查找对应的SkeletalMeshActor
     * @param SkeletalMeshComponent 骨骼网格组件
     * @return 对应的SkeletalMeshActor，未找到返回nullptr
     */
    static ASkeletalMeshActor* FindSkeletalMeshActorFromComponent(
     ...
     *
     * @param LevelSequence Level Sequence
     * @param ObjectBindingID
     * 绑定的对象GUID（通常是SkeletalMeshComponent的绑定）
    ...
    FindOrCreateComponentMaterialTrack(ULevelSequence* LevelSequence,
     ...
     */
    static bool AddMaterialParameter(UMovieSceneComponentMaterialTrack* Track,
     ...
     */
    static FGuid GetOrCreateComponentBinding(TSharedPtr<ISequencer> Sequencer,
 ...
 * 提供静态方法的向后兼容包装
 */
class MUSICDOLLCOMMON_API FInstrumentAnimationUtility {
   public:
    static void UnwrapRotationSequence(
        TArray<FMovieSceneFloatValue>& RotationValues) {
    ...
    static FMovieSceneFloatChannel* FindFloatChannel(
        UMovieSceneSection* Section, const FString& ChannelName) {
        return UInstrumentAnimationUtility::FindFloatChannel(Section,
                                                             ChannelName);
    }
