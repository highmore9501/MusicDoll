...
struct COMMON_API FMorphTargetKeyframeData
{
    ...
    FMorphTargetKeyframeData()
        : MorphTargetName(TEXT(""))
    {
    }
 ...
 * UInstrumentMorphTargetUtility::GetMorphTargetNames(SkeletalMeshComp, MorphTargetNames);
 * 
 * // 确保Root Control存在
...
public:
     ...
     */
    static bool GetMorphTargetNames(
    ...
    /**
     * 从JSON文件解析Morph Target关键帧数据
     * 
     * 支持两种JSON格式:
     * 1. 数组格式: [{shape_key_name, keyframes: [{frame, shape_key_value}]}]
     * 2. 对象格式: {keys: [{shape_key_name, keyframes: [{frame, shape_key_value}]}]}
     * 
     * @param JsonFilePath JSON文件路径
     * @param OutKeyframeData 输出：关键帧数据数组
     * @param TickResolution Tick分辨率（用于帧数转换）
     * @param DisplayRate 显示帧率（用于帧数转换）
     * @return 是否成功解析（至少有一个Morph Target数据）
     * 
     * @note 帧数转换公式: ScaledFrame = Frame * TickRes.Num * DisplayRate.Den / (TickRes.Den * DisplayRate.Num)
     * @note 如果同一个Morph Target在JSON中出现多次，关键帧会被追加（不会覆盖）
     */
    static bool ParseMorphTargetJSON(
